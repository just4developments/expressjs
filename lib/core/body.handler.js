...
global.MIME_JSON_ENCRYPT = 'encryption/json';

exports = module.exports = {
  encrypt(encryptKey=')^!!!(**') {
    encrypt.blowfish(encryptKey);
    return (req, res, next) => {
      if(req.headers['accept'] && req.headers['accept'].includes(MIME_JSON_ENCRYPT)) {
        req.encrypt = encryptKey;
        if(req.query.q) {
          let query = encrypt.decryptText(req.query.q);
        ...
        res.contentType(MIME_JSON_ENCRYPT);
        const _send = res.send;
        res.send = (data) => {
          _send.apply(res, [encrypt.encryptText(data)]);
        }
      }
      next();
    };
  ...
  jsonHandler(schema){
    ...
    mid.push((req, res, next) => {
      if(req.encrypt) {
        ...
        req.on('end', function() {
          req.body = JSON.parse(encrypt.decryptText(chunks.join()));
          next();
        });
      } else {
    ...
    if(schema) mid.push(exports.validateBodyHandler(schema));          
    return mid;
  },
  fileHandler(schema){
  ...
  validateBodyHandler(schema){
    return async (req, res, next) => {      
      let applyValue = async (schema, body) => {
        let tmp = {};
        for(let k in schema) {
          if(!_.isNil(body[k])) {
            if(schema[k] === String) {
              tmp[k] = body[k].toString();
            } else if(schema[k] === Number) {
              tmp[k] = +body[k];
            } else if(schema[k] === Object || schema[k] === Array) {
              tmp[k] = cast.object(body[k]);
            } else if(schema[k] === Boolean) {
              tmp[k] = cast.boolean(body[k]);
            } else if(schema[k] === Date) {
              tmp[k] = cast.date(body[k]);
            } else if(typeof schema[k] === 'function' && !_.isNil(body[k])) {
              tmp[k] = schema[k](body[k]);
            } else {
              tmp[k] = body[k];
            }
      ...
      if(schema instanceof Array) {
        ...
        for(let k in req.body){ 
          vl.push(await applyValue(schema[0], req.body[k]));
        }
        req.body = vl;
      } else {
        req.body = await applyValue(schema, req.body);
      }
      next();
    }
  },
